...
const char *executeCommand(const char *command) {
  ...
  // Vietnamese labels: quay/dung -> FAN_PIN, bat/tat -> LIGHT_PIN
  if (strcmp(command, "quay") == 0 || strcmp(command, "fan_on") == 0) {
    gpio_set_level(static_cast<gpio_num_t>(FAN_PIN), 1);
    return "Fan ON";
  }
  if (strcmp(command, "dung") == 0 || strcmp(command, "fan_off") == 0) {
    gpio_set_level(static_cast<gpio_num_t>(FAN_PIN), 0);
    return "Fan OFF";
  }
  if (strcmp(command, "bat") == 0 || strcmp(command, "light_on") == 0) {
    gpio_set_level(static_cast<gpio_num_t>(LIGHT_PIN), 1);
    return "Light ON";
  }
  if (strcmp(command, "tat") == 0 || strcmp(command, "light_off") == 0) {
